...
def findratio():
    ...
    # Loop through each peer in the network
    longestchaintotalblockslist = []
    totalblocksgenerated = 0
    for i in simulator.peerslist:
        peerid = i.pid
        parent = i.TopBlockHash

        Advgenratednodeslongestchain = 0
        Advgenratednodes = 0
        
        if simulator.peerslist.index(i) == simulator.attackernode:
            #Nodes generated by adversary in longest chain.
            while(parent != 'Root'):
                generatorofblock = i.BlockTree[parent][0]['generator']
                if peerid == generatorofblock:
                    Advgenratednodeslongestchain += 1
                parent = i.BlockTree[parent][1]

            #calculate total blocks in blockchain by Adversary peer 
            for hash,block in i.BlockTree.items():
                generatorofblock = block[0]['generator']
                if peerid == generatorofblock:
                        Advgenratednodes += 1
            
            print(f"MPU of Adversary: {Advgenratednodeslongestchain/Advgenratednodes}")
        ...
        while(parent != 'Root'):
                ...
                parent = i.BlockTree[parent][1]
        longestchaintotalblockslist.append(longestchaintotalblocks)
    ...
    count = 0
    for i in longestchaintotalblockslist:
        print()
        if totalblocksgenerated != 0:
            print(f"MPU Overall for Node {count}: {i/totalblocksgenerated}")
            count += 1
...
if __name__ == "__main__":
    ...
    # count = 0
    # for i in simulator.peerslist:
    #     print(count, i.isstubborn, i.lead, i.HashPower, i.is_slow, i.is_low_cpu)
    #     count += 1
    exit(0)
