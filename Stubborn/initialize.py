...
class initialize_parameters:
    ...
    def __init__(self, path):
        ...
        self.attackmode = self.findattackmode(int(parameter_list[0][4]))
        
        self.advhashpower = int(parameter_list[0][5])
        self.advconnected = int(parameter_list[0][6])
        # Set the value of Tmean
        self.Tmean = [int(parameter_list[1][0])] * self.Nodes
        # Set the value of Kmean
        self.Kmean = [int(parameter_list[2][0])] * self.Nodes
        # Set SlowNodesList
        self.SlowNodesList = self.setnodes(self.Z0)
        # Set LowCPUNodesList
        self.LowCPUNodesList = self.setnodes(self.Z1)
        self.attackernode = self.findattacker()
        # Set the value of HashingpowerSlow
        self.HashingpowerSlow = self.FindOutPower(self.advhashpower)
        self.peerIds = []
        # Set the value of peerslist
        self.peerslist = self.setpeers()
        # Set the value of graph
        self.graph = self.creategraph(self.advconnected)
    ...
    def findattackmode(self,var):
        if var == 0:
            ...
            print("Enter Valid Attack Type")
            exit(0)

    def FindOutPower(self,val):
        power = (1 - (val/100)) /((10*self.Nodes) - (9*len(self.LowCPUNodesList)) - 10)
        return power
    ...
    def findattacker(self,):
        randomNo = random.randint(0,self.Nodes)
        while(randomNo in self.SlowNodesList):
            randomNo = random.randint(0,self.Nodes)
        return randomNo
    ...
    def setpeers(self):
        ...
        isslow = False
        islowcpu = False
        attackernode = self.attackernode
        print(self.SlowNodesList)
        print(self.LowCPUNodesList)
        #loop through total number of peers
        for i in range(self.Nodes):
            ishonest = True
            isstubborn = False
            HashingpowerSlow = self.HashingpowerSlow
            isslow = False
            islowcpu = False
            ...
            if i in self.LowCPUNodesList:
                islowcpu = True
            if i == attackernode:
                if self.attackmode == 'selfish':
                    ishonest = False
                elif self.attackmode == 'stubborn':
                    isstubborn = True
                HashingpowerSlow = self.advhashpower/1000
            ...
            peers.append(Peer(peer_id, isslow, islowcpu, self.Tmean[i], self.Kmean[i], HashingpowerSlow, ishonest, isstubborn))
        return peers
    ...
    def creategraph(self, advconnected):
        ...
        for currentNodeindex in range(self.Nodes):
            ...
            if currentNodeindex == self.attackernode:
                num_edges = int(advconnected * (self.Nodes)/100)
